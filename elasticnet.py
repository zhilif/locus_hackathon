...
# Elastic net loss
def elastic_loss(linear, X, y, lam, alpha, family='multinomial', sample_weight=None, fit_intercept=False):
    weight = None
    bias = None
    if fit_intercept:
        weight, bias = list(linear.parameters())
    else:
        weight = list(linear.parameters())[0]
...
def elastic_loss_loader(linear, loader, lam, alpha, preprocess=None, family='multinomial', fit_intercept=False):
    loss = 0
    n = 0
    ...
    for batch in loader:
        ...
        if preprocess is not None:
            X, y = preprocess(X, y)
        bs = X.size(0)
        loss += elastic_loss(linear, X, y, lam, alpha, family=family, fit_intercept=fit_intercept)*bs
        n += bs
    return loss/n

# Elastic net loss and accuracy
def elastic_loss_and_acc(linear, X, y, lam, alpha, family='multinomial', mask=None, fit_intercept=False):
    weight = None
    bias = None
    if fit_intercept:
        weight, bias = list(linear.parameters())
...
# Elastic net loss given a loader instead
def elastic_loss_and_acc_loader(linear, loader, lam, alpha, preprocess=None, family='multinomial', mask=None, fit_intercept=None):
    loss = 0
    acc=0
    n = 0
    device = linear.weight.device
    ...
    for batch in loader:
        ...
        if preprocess is not None:
            X, y = preprocess(X, y)
        bs = X.size(0)
        l,a = elastic_loss_and_acc(linear, X, y, lam, alpha, family=family, mask=mask, fit_intercept=fit_intercept)
        loss += l*bs
        acc += a*bs
        n += bs
    return loss/n, acc/n
...
def train(linear, X, y, lr, niters, lam, alpha, group=True, verbose=None, fit_intercept=False):
    weight = None
    bias = None
    if fit_intercept:
        weight, bias = list(linear.parameters())
...
def train_saga(linear, loader, lr, nepochs, lam, alpha, group=True, verbose=None,
                ...
                preprocess=None, lookbehind=None, family='multinomial', logger=None, fit_intercept=False):
    if logger is None:
        logger = print
    with ch.no_grad():
        weight = None
        ...
        for t in range(nepochs):
            total_loss = 0
            # tk2 = tqdm(loader,position = 0, leave=False)
            for batch in loader:
                if len(batch) == 3:
                    X,y,idx = batch
                    # if (len(X.shape) == 3):
                    #     idx = ch.arange(500*idx.item(), 500*idx.item()+X.shape[1])
                    w = None
                elif len(batch) == 4:
                    X,y,w,idx = batch
                else:
                ...
                if family == 'multinomial':
                    if w is None:
                        ...
                        loss = (loss*w).mean()
                    I = ch.eye(linear.weight.size(0)).to(y.device)
                ...
                # print(w_grad.shape, w_grad_prev.shape, w_grad_avg.shape)
                ...
                if lookbehind is None:
                    ...
                    # tk2.set_postfix({'diff': criteria.item()})
                    if criteria.item() <= tol:
                        logger(f"converges in iteration {t}")
                        return {
                            "a_table": a_table.cpu(),
                            "w_grad_avg": w_grad_avg.cpu(),
                            "b_grad_avg": b_grad_avg.cpu() if fit_intercept else None
                        }
...
def glm_saga(linear, loader, max_lr, nepochs, alpha,
             ...
             tol=1e-4, lam_multiplier=1, epsilon=0.001, k=100, checkpoint=None,
             do_zero=True, lr_decay_factor=1, metadata=None,
             val_loader=None, test_loader=None, lookbehind=None,
             family='multinomial', encoder=None, max_lam=None, preprocess_val=None, preprocess_test=None, fit_intercept=False):
    ...
    if max_lam:
        max_lam *= lam_multiplier
    if max_lam is None:
        max_lam = maximum_reg_loader(loader, group=group, preprocess=preprocess, metadata=metadata, family=family) / max(0.001, alpha) * lam_multiplier
    min_lam = epsilon*max_lam
    print(f'MAX LAM {max_lam}')
    ...
    for i,(lam,lr) in enumerate(zip(lams,lrs)):
        ...
        state = train_saga(linear, loader, lr, nepochs, lam, alpha,
                    ...
                    family=family, logger=logger, fit_intercept=fit_intercept)

        with ch.no_grad():
            loss,acc = elastic_loss_and_acc_loader(linear, loader, lam, alpha, preprocess=preprocess, family=family, fit_intercept=fit_intercept)
            loss,acc = loss.item(),acc.item()

            loss_val,acc_val = -1,-1
            if val_loader:
                loss_val,acc_val = elastic_loss_and_acc_loader(linear, val_loader, lam, alpha, preprocess=preprocess_val, family=family, fit_intercept=fit_intercept)
                loss_val,acc_val = loss_val.item(),acc_val.item()

            loss_test,acc_test = -1,-1
            if test_loader:
                loss_test,acc_test = elastic_loss_and_acc_loader(linear, test_loader, lam, alpha, preprocess=preprocess_test, family=family, fit_intercept=fit_intercept)
            ...
            params = {
                ...
                "bias": linear.bias.detach().cpu().clone() if fit_intercept else None

            }
            path.append(params)
            if acc_val is not None and acc_val > best_val_acc:
...
class GLM():
    def __init__(self, batch_size=128, val_frac=0.1, lr=0.1, max_epochs=2000,
                alpha=1, verbose=200, group=False, lam_factor=0.001, tol=1e-4, lam_multiplier=1, max_lam=None):
